...
export default {
	...
	async run(date, moda, env) {
		...
		if (event.startDate.startsWith(date)) {
			await this.discord(discordUrl, `**${event.name}** at the Moda Center today (${date})! please park with caution`);
		} else {
			await this.discord(discordUrl, `nothing at the Moda Center today (${date}) :)`);
		}
	},
};
